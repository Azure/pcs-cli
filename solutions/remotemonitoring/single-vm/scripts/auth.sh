...
_set_keyvault_secret() {
    ...
    _keyvault_secret_bundle=$(curl -v -X PUT -H "Content-Type: application/json" -H "Authorization: Bearer $AUTH_TOKEN" -d "{\"value\": $1}"  -L https://$PCS_KEYVAULT_NAME.vault.azure.net/secrets/authRequired/?api-version=7.0)
